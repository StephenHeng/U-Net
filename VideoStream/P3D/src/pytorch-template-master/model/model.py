...
class P3D(BaseModel):
    ...
    def __init__(self, block, layers, modality='RGB',
                 shortcut_type='B', num_classes=50, dropout=0.5, ST_struc=('A', 'B', 'C')):
        self.inplanes = 64
        ...
        self.layer1 = self._make_layer(64, layers[0], shortcut_type)        # 3
        self.layer2 = self._make_layer(128, layers[1], shortcut_type, stride=2)     # 4
        self.layer3 = self._make_layer(256, layers[2], shortcut_type, stride=2)     # 6
        self.layer4 = self._make_layer(512, layers[3], shortcut_type, stride=2)     # 3

        self.avgpool = nn.AvgPool2d(kernel_size=(5, 5), stride=1,padding=1)  # pooling layer for res5.
        self.dropout = nn.Dropout(p=dropout)
        self.fc = nn.Linear(512 * 4*4*4, num_classes)
    ...
    def _make_layer(self, planes, blocks, shortcut_type, stride=1):
        downsample = None
            ...
            if stride != 1 or self.inplanes != planes * Bottleneck.expansion:
                if shortcut_type == 'A':
                    ...
                    downsample = nn.Sequential(
                        nn.Conv3d(self.inplanes, planes * Bottleneck.expansion,
        ...
        layers = []
        layers.append(Bottleneck(self.inplanes, planes, stride, downsample, n_s=self.cnt, depth_3d=self.depth_3d,
                            ST_struc=self.ST_struc))
        self.cnt += 1

        self.inplanes = planes * Bottleneck.expansion
        for i in range(1, blocks):
            layers.append(Bottleneck(self.inplanes, planes, n_s=self.cnt, depth_3d=self.depth_3d, ST_struc=self.ST_struc))
            self.cnt += 1
...
# #         }
class C3D(BaseModel):
    def __init__(self, num_seg_class=50, temporal_channel=16):
        super(C3D, self).__init__()
        self.conv3d1 = nn.Conv3d(
            3, 256, (3, 5, 5), stride=(1, 3, 3))  # [256,14,,]
        self.pool1 = nn.MaxPool3d((2, 2, 2))  # [256,7,,]
        self.conv3d2 = nn.Conv3d(256, 512, (3, 3, 3), stride=1)  # [256,5,,]
        self.pool2 = nn.MaxPool3d((1, 2, 2))
        self.conv3d3 = nn.Conv3d(512, 256, (3, 3, 3), stride=1)  # [256,3,,]
        self.pool3 = nn.MaxPool3d((1, 2, 2))

        self.conv3d4 = nn.Conv3d(256, 256, (3, 3, 3), stride=1)  # [256,1,,]

        self.conv3d5 = nn.Conv3d(256, 256, (1, 3, 3), stride=1)  # [256,1,,]
        # self.pool3 = nn.MaxPool3d((1, 2, 2))                     # [256,1,,]

        self.gpl1 = nn.AdaptiveAvgPool3d((1, 1, 1))

        self.line1 = nn.Linear(256, 128)
        self.line2 = nn.Linear(128, num_seg_class)

    def forward(self, x):
        # print(x.size())
        x = x.float()
        x = self.conv3d1(x)  # [256,14,,]
        x = self.pool1(x)  # [256,7,,]
        x = self.conv3d2(x)  # [256,5,,]
        x = self.pool2(x)
        x = self.conv3d3(x)  # [256,3,,]
        x = self.pool3(x)
        x = self.conv3d4(x)  # [256,1,,]
        x = self.conv3d5(x)  # [256,1,,]
        x = self.gpl1(x)
        x = x.view(-1, 256)
        x = self.line1(x)
        x = self.line2(x)
